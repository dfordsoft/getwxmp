...
type Options struct {
	Verbose           bool   `short:"v" long:"verbose" description:"should every proxy request be logged to stdout (default: false)"`
	DisableProxyLog   bool   `long:"disable-proxy-log" description:"disable proxy logs (default: true)"`
	UpdateProxyOnly   bool   `short:"p" long:"update-proxy-only" description:"update proxy list only then exit immediately (default: false)"`
	DirectConnecting  bool   `short:"d" long:"direct-connecting" description:"download articles/image without proxy (default: true)"`
	UseLastFrameOfGIF bool   `long:"last-frame" description:"use last frame of GIF image (default: true)"`
	...
	ReverseOrder      bool   `short:"r" long:"reverse-order" description:"put older articles in front (default: false)"`
